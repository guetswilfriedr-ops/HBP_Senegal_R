...
# GBD-cause prevalence-by-quintile row (tab 2), a coverage/utilization
# indicator (tab 3), a national opportunity-cost distribution (tab 4),
# and the CET/opportunity-cost/inequality-aversion parameters (tab 5).
#
...
#'     cause_id, cause_gbd, n_interventions, q1..q5 (share of cases, %),
#'     sum_check, source, year, indicator, page, justification, status, tier
#'   e_table - one row per coverage indicator (tab 3), columns:
#'     indicator_id, indicator_name, description, q1..q5 (rate, %),
#'     source, year, page, justification, status, tier
#'   f_row - the single national opportunity-cost row (tab 4), columns:
#'     q1..q5 (share, %), sum_check, source, year, indicator, page,
...
read_dcea_prep <- function(path) {
  ...
  incomplete_d <- d_table$cause_gbd[is.na(d_table$q1) | is.na(d_table$q5)]
  if (length(incomplete_d) > 0) {
    warning(
      "2_D_prevalence_by_cause has incomplete quintile shares for: ",
...
#' (config$cet_usd_per_daly) with a warning, since the CET used to rank
